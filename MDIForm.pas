...
uses
  ...
  SESLabIO, ced1902u, ComCtrls, ADCDataFile, strutils, math, StdCtrls,
  FileCtrl ;
...
begin
     ...
     // Stimulus protocols folder
      ...
      if not DirectoryExists(Settings.VProtDirectory) then begin
         if ForceDirectories(Settings.VProtDirectory) then
      ...
      if not DirectoryExists(Settings.DataDirectory) then begin
         if ForceDirectories(Settings.DataDirectory) then
      ...
      WCPClipboardFileName := TempPath + 'WCPClipboardFile.tmp' ;
...
begin
     ...
     SaveDialog.FileName := CreateIndexedFileName( {ExtractFileName( }RawFH.FileName {)}) ;
...
var
   ...
   i,j,j0 : Integer ;
   NumSamplesPerBuf : Integer ; // No. of A/D samples in I/O buffer
   Buf : ^TSmallIntArray ;
   NumScans : Integer ;
   NumRead : Integer ;
   NumWrite : Integer ;
...
begin
     ...
     //RawFH.NumBytesInHeader := MaxBytesInFileHeader ;
     // Time created
     RawFH.CreationTime := DateTimeToStr(Now) ;
     RawFH.NumRecords := 0 ;
...
begin
     ...
     FileName := ExtractFilePath(FileName) + sDate + '.wcp' ;
...
end. procedure TMain.o(Sender: TObject);
begin
